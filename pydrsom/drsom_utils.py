...
def query_name(optimizer, name, args, ckpt):
    if name.startswith("drsom"):
        return (
            f"{optimizer.get_name()}"
            if not args.resume
            else f"{optimizer.get_name()}-r-{ckpt['epoch']}"
        )
    else:
        return name
    ...
    def _solve_alpha(optimizer, Q, c, G):
        ...
        if optimizer.iter == 0 or optimizer.G[dim - 1, dim - 1] < 1e-4:
            ...
            if optimizer.qpsolver in {DRSOMQPSolver.TRSP, DRSOMQPSolver.TRSA}:
                it, lmd, alpha, norm, active = TRS._compute_root_tr(
                    Q, c, optimizer.radius, G
                )
            else:
                it, lmd, alpha, norm, active = TRS._compute_root(
                    Q, c, optimizer.gamma, G
                )
        ...
        if DRSOM_VERBOSE:
            optimizer.logline = {
                **optimizer.logline,
                "𝜆": "{:+.2e}".format(lmd),
                "Q/c/G": np.round(
                    np.vstack([optimizer.Q, optimizer.c, optimizer.G]), 6
                ),
...
def add_parser_options(parser: ArgumentParser):
    ...
    parser.add_argument(
        ...
        default=-1,
        help=f"""
           whether to use a fixed momentum like the adaptive gradient method,
           if it is  > 0, then adopt this
           if the qp direction uses the momentum, this will not be used.
          """,
    )
    parser.add_argument("--drsom_solve_qp", required=False, type=int, default=0)
    parser.add_argument("--drsom_normalize", required=False, type=int, default=0)
    ##############
    ...
    # decay rules
    parser.add_argument(
        "--drsom_decay_mode", default=-1, type=float, help="""see drsom_decay_window"""
    )
    parser.add_argument(
        "--drsom_decay_window",
        default=4000,
    ...
    parser.add_argument(
        "--drsom_decay_step", default=2e1, type=float, help="see drsom_decay_window"
    )
    parser.add_argument(
...
class DRSOMDecayRules(object):
    def __init__(self, **kwargs):
        ...
        # linear rule
        self.decay_mode = DRSOMModeDecay(kwargs.get("decay_mode", 2))
        self.decay_window = kwargs.get("decay_window", 4000)
        self.decay_step = kwargs.get("decay_step", 1e2)
    ...
    def __str__(self):
        return f"[{self.decay_mode.name}w@{self.decay_sin_rel_max:.2e}:{self.decay_sin_min_scope:.2e}-{self.decay_sin_max_scope:.2e}]"
    ...
    def adjust_gamma_and_radius(self, opt):
        if self.decay_mode == DRSOMModeDecay.Sin:
            ...
            opt.gammalb = (
                opt.gammalb0
                * self.decay_sin_rel_max
                / (1 + np.exp(10 - 20 * (opt.iter + 1) / self.decay_sin_max_scope))
            )
        elif self.decay_mode == DRSOMModeDecay.Lin:
            if ((opt.iter + 1) % self.decay_window) == 0:
                # using the following formulae
                # gamma = gamma_0 * gamma_max *
                #   1/{1 + exp[-10 + 20/M*(x)]}
                ...
                # - M total iterations
                # - gamma_0, gamma_max, initial and target
                opt.gammalb *= self.decay_step
                opt.gammalb = min(opt.gammalb, 1e-3)
        ...
        if ((opt.iter + 1) % 1000) == 0:
            print(
                f"current regularization terms: {opt.gammalb} and {opt.radiusub} @{opt.iter}\n"
